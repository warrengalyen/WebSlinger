...
class WebBrowser {
			...
			switch ($state["state"]) {
				case "initialize":
				{
					if (!isset($this->data["allowedprotocols"][$state["urlinfo"]["scheme"]]) || !$this->data["allowedprotocols"][$state["urlinfo"]["scheme"]]) {
						return array(
							"success"   => false,
							"error"     => self::WBTranslate("Protocol '%s' is not allowed in '%s'.", $state["urlinfo"]["scheme"], $state["url"]),
							...
							foreach ($paths as $path => $cookies2) {
								if (substr($cookiepath, 0, strlen($path)) == $path) {
									foreach ($cookies2 as $num => $info) {
										...
										} else if ($secure || !isset($info["secure"])) {
					...
					if ($state["timeout"] !== false) {
						$state["options"]["timeout"] = HTTP::GetTimeLeft($state["startts"], $state["timeout"]);
					}
					...
					if (isset($state["options"]["pre_retrievewebpage_callback"]) && is_callable($state["options"]["pre_retrievewebpage_callback"]) && !call_user_func_array($state["options"]["pre_retrievewebpage_callback"], array(&$state))) {
						return array(
							"success"   => false,
							"error"     => self::WBTranslate("Pre-RetrieveWebpage callback returned with a failure condition for '%s'.", $state["url"]),
					...
					if (!$result["success"]) {
						return array(
							"success"   => false,
							"error"     => self::WBTranslate("Unable to retrieve content.  %s", $result["error"]),
							"info"      => $result,
							"state"     => $state,
				...
				case "post_retrieval":
				{
					...
					if (isset($state["result"]["headers"]["Location"]) && $this->data["followlocation"]) {
						...
						if (!isset($this->data["allowedredirprotocols"][$urlinfo2["scheme"]]) || !$this->data["allowedredirprotocols"][$urlinfo2["scheme"]]) {
							return array(
								"success"   => false,
								"error"     => self::WBTranslate("Protocol '%s' is not allowed.  Server attempted to redirect to '%s'.", $urlinfo2["scheme"], $state["url"]),
								"info"      => $state["result"],
					...
					if (isset($state["result"]["headers"]["Location"]) && $this->data["followlocation"] && $state["numfollow"]) {
						...
						if ($this->data["extractforms"]) {
							$state["result"]["forms"] = $this->ExtractForms($state["result"]["url"], $state["result"]["body"], (isset($state["tempoptions"]["extractforms_hint"]) ? $state["tempoptions"]["extractforms_hint"] : false));
						}
	...
	public function Process($url, $tempoptions = array()) {
		...
		} else if (isset($this->data["httpopts"]["timeout"])) {
		...
		} else if (isset($this->data["httpopts"]["streamtimeout"])) {
	...
	public function ProcessAsync__Handler($mode, &$data, $key, &$info) {
		switch ($mode) {
			...
			case "readfps":
			{
				if ($info["state"]["httpstate"] !== false && HTTP::WantRead($info["state"]["httpstate"])) {
			...
			case "writefps":
			{
				if ($info["state"]["httpstate"] !== false && HTTP::WantWrite($info["state"]["httpstate"])) {
					$data[$key] = $info["state"]["httpstate"]["fp"];
				}
	...
	public function ExtractForms($baseurl, $data, $hint = false) {
		...
		foreach ($rows as $row) {
			...
			foreach ($rows2 as $row2) {
				if (!isset($row2->form)) {
					if (isset($row2->id) && $row2->id != "" && isset($hintmap[trim($row2->id)])) {
	...
	private function ExtractFieldFromDOM(&$fields, $row, &$lasthint) {
		switch ($row->tag) {
			case "input":
			{
				if (!isset($row->name) && ($row->type === "submit" || $row->type === "image")) {
					$row->name = "";
				}
				...
				if (isset($row->name) && is_string($row->name)) {
					...
					} else if ($field["type"] == "input.submit" || $field["type"] == "input.image") {
	...
	public static function InteractiveFormFill($forms, $showselected = false) {
		...
		foreach ($form->fields as $num => $field) {
			if ($field["type"] != "input.submit" && $field["type"] != "input.image" && $field["type"] != "input.button" && $field["type"] != "button.submit") {
				continue;
			}
	...
	public function SetCookie($cookie) {
		if (!isset($cookie["domain"]) || !isset($cookie["path"]) || !isset($cookie["name"]) || !isset($cookie["value"])) {
			return array(
				"success"   => false,
				"error"     => self::WBTranslate("SetCookie() requires 'domain', 'path', 'name', and 'value' to be options."),
			...
			if ($domainpattern == "" || substr($domain, - strlen($domainpattern)) == $domainpattern) {
				foreach ($paths as $path => $cookies) {
					if ($pathpattern == "" || substr($path, 0, strlen($pathpattern)) == $pathpattern) {
						foreach ($cookies as $num => $info) {
							if ($namepattern == "" || strpos($info["name"], $namepattern) !== false) {
								unset($this->data["cookies"][$domain][$path][$num]);
							}
						}
...
class WebBrowserForm {
	...
	public function GetVisibleFields($submit) {
		$result = array();
		foreach ($this->fields as $num => $field) {
			if ($field["type"] == "input.hidden" || (!$submit && ($field["type"] == "input.submit" || $field["type"] == "input.image" || $field["type"] == "input.button" || substr($field["type"], 0, 7) == "button."))) {
				continue;
			}
