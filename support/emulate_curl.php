<?php
// HTTP cURL emulation functions.

// cURL HTTP emulation support requires:
//   The HTTP functions.
//   The Web Browser state emulation class.
if (!function_exists("curl_init")) {
	...
	function curl_init($url = false) {
		...
		$curl_init__map[$key] = array("self"      => $ch,
		                              "method"    => "GET",
		                              "options"   => $options,
	...
	function curl_exec($ch) {
		...
		if (isset($curl_init__map[$key]["options"][CURLOPT_HTTP_VERSION])) {
			if ($curl_init__map[$key]["options"][CURLOPT_HTTP_VERSION] == CURL_HTTP_VERSION_1_0) {
				$options["httpver"] = "1.0";
			} else if ($curl_init__map[$key]["options"][CURLOPT_HTTP_VERSION] == CURL_HTTP_VERSION_1_1) {
				$options["httpver"] = "1.1";
			}
		}
		...
		if (isset($curl_init__map[$key]["options"][CURLOPT_TIMECONDITION]) && isset($curl_init__map[$key]["options"][CURLOPT_TIMEVALUE])) {
			...
			} else if ($curl_init__map[$key]["options"][CURLOPT_TIMECONDITION] == CURL_TIMECOND_IFUNMODSINCE) {
	...
	function internal_curl_debug_callback($type, $data, $key) {
		...
		if ($output !== "" && isset($curl_init__map[$key]["options"][CURLOPT_VERBOSE]) && $curl_init__map[$key]["options"][CURLOPT_VERBOSE]) {
			if (isset($curl_init__map[$key]["options"][CURLOPT_STDERR]) && is_resource($curl_init__map[$key]["options"][CURLOPT_STDERR])) {
				fwrite($curl_init__map[$key]["options"][CURLOPT_STDERR], $output);
	...
	function internal_curl_write_body_callback(&$body, &$bodysize, $key) {
		...
		if (!$bodysize) {
			...
			if (!isset($curl_init__map[$key]["options"][CURLOPT_INFILE]) || !is_resource($curl_init__map[$key]["options"][CURLOPT_INFILE])) {
				return false;
			}
			if ($bodysize > 32768) {
				$body = fread($curl_init__map[$key]["options"][CURLOPT_INFILE], 32768);
			} else {
				$body = fread($curl_init__map[$key]["options"][CURLOPT_INFILE], $bodysize);
			}
			if ($body === false) {
				if (isset($curl_init__map[$key]["options"][CURLOPT_VERBOSE]) && $curl_init__map[$key]["options"][CURLOPT_VERBOSE]) {
	...
	function internal_curl_read_headers_callback(&$response, &$headers, $key) {
		...
		if ($response["code"] >= 400 && isset($curl_init__map[$key]["options"][CURLOPT_FAILONERROR]) && $curl_init__map[$key]["options"][CURLOPT_FAILONERROR]) {
			return true;
		}
		...
		if (isset($curl_init__map[$key]["options"][CURLOPT_HEADER]) && $curl_init__map[$key]["options"][CURLOPT_HEADER]) {
			if (isset($curl_init__map[$key]["options"][CURLOPT_VERBOSE]) && $curl_init__map[$key]["options"][CURLOPT_VERBOSE]) {
				if (isset($curl_init__map[$key]["options"][CURLOPT_STDERR]) && is_resource($curl_init__map[$key]["options"][CURLOPT_STDERR])) {
					fwrite($curl_init__map[$key]["options"][CURLOPT_STDERR], HTTP::HTTPTranslate("Header:\n") . $data);
				} else if (defined("STDERR")) {
		...
		if (!isset($headers["Location"]) || !isset($curl_init__map[$key]["options"][CURLOPT_FOLLOWLOCATION]) || !$curl_init__map[$key]["options"][CURLOPT_FOLLOWLOCATION]) {
			if (isset($curl_init__map[$key]["options"][CURLOPT_HEADER]) && $curl_init__map[$key]["options"][CURLOPT_HEADER]) {
				if (isset($curl_init__map[$key]["options"][CURLOPT_WRITEHEADER]) && is_resource($curl_init__map[$key]["options"][CURLOPT_WRITEHEADER])) {
					...
					if (isset($curl_init__map[$key]["options"][CURLOPT_FILE]) && is_resource($curl_init__map[$key]["options"][CURLOPT_FILE])) {
						fwrite($curl_init__map[$key]["options"][CURLOPT_FILE], $data);
					} else {
						echo $data;
					}
				}
	...
	function internal_curl_read_body_callback(&$response, $data, $key) {
		...
		if (!isset($headers["Location"]) || !isset($curl_init__map[$key]["options"][CURLOPT_FOLLOWLOCATION]) || !$curl_init__map[$key]["options"][CURLOPT_FOLLOWLOCATION]) {
			...
			if (isset($curl_init__map[$key]["options"][CURLOPT_VERBOSE]) && $curl_init__map[$key]["options"][CURLOPT_VERBOSE]) {
				if (isset($curl_init__map[$key]["options"][CURLOPT_STDERR]) && is_resource($curl_init__map[$key]["options"][CURLOPT_STDERR])) {
					fwrite($curl_init__map[$key]["options"][CURLOPT_STDERR], HTTP::HTTPTranslate("Body:\n") . $data);
				} else if (defined("STDERR")) {
