...
// Requires HTTP/HTTPS class.
// Requires WebBrowser class.
class WebSocket {
	...
	public function Connect($url, $origin, $options = array(), $web = false) {
		...
		if (isset($options["connected_fp"]) && is_resource($options["connected_fp"])) {
			...
			if ($url2["scheme"] != "ws" && $url2["scheme"] != "wss") {
				return array("success"   => false,
				             "error"     => self::WSTranslate("WebSocket::Connect() only supports the 'ws' and 'wss' protocols."),
			...
			if ($result["response"]["code"] != 101) {
				return array("success"   => false,
				             "error"     => self::WSTranslate("WebSocket::Connect() failed to connect to the WebSocket.  Server returned:  %s %s", $result["response"]["code"], $result["response"]["meaning"]),
				             "errorcode" => "incorrect_server_response"
				);
			}
			if (!isset($result["headers"]["Sec-Websocket-Accept"])) {
				return array("success"   => false,
				             "error"     => self::WSTranslate("Server failed to include a 'Sec-WebSocket-Accept' header in its response to the request."),
			...
			if ($result["headers"]["Sec-Websocket-Accept"][0] !== base64_encode(sha1($key . self::KEY_GUID, true))) {
				return array("success"   => false,
				             "error"     => self::WSTranslate("The server's 'Sec-WebSocket-Accept' header is invalid."),
	...
	public function Disconnect() {
		if ($this->fp !== false && $this->state === self::STATE_OPEN) {
			if ($this->closemode === self::CLOSE_IMMEDIATELY) {
				$this->writemessages = array();
			} else if ($this->closemode === self::CLOSE_AFTER_CURRENT_MESSAGE) {
	...
	public function Write($message, $frametype, $last = true, $wait = false, $pos = false) {
		...
		if ($frametype >= 0x08 || $lastcompleted) {
			...
			if ($frametype !== $this->writemessages[$y - 1]["opcode"]) {
				return array("success"   => false,
				             "error"     => self::WSTranslate("Mismatched frame type (opcode) specified."),
	...
	public function ProcessQueuesAndTimeoutState($read, $write, $readsize = 65536) {
		...
		if ($read) {
			...
			if ($result === false || ($result === "" && feof($this->fp))) {
				return array("success"   => false,
				             "error"     => self::WSTranslate("ProcessQueuesAndTimeoutState() failed due to fread() failure.  Most likely cause:  Connection failure."),
			...
			if ($result !== "") {
				...
				if ($this->maxreadframesize !== false && strlen($this->readdata) > $this->maxreadframesize) {
					return array("success"   => false,
					             "error"     => self::WSTranslate("ProcessQueuesAndTimeoutState() failed due to peer sending a single frame exceeding %s bytes of data.", $this->maxreadframesize),
		...
		if ($this->lastkeepalive < time() - $this->keepalive) {
			if ($this->keepalivesent) {
				return array("success"   => false,
				             "error"     => self::WSTranslate("ProcessQueuesAndTimeoutState() failed due to non-response from peer to ping frame.  Most likely cause:  Connection failure."),
	...
	protected function ProcessReadData() {
		while (($frame = $this->ReadFrame()) !== false) {
			...
			if (($frame["opcode"] >= 0x03 && $frame["opcode"] <= 0x07) || $frame["opcode"] >= 0x0B) {
				return array("success"   => false,
				             "error"     => self::WSTranslate("Invalid frame detected.  Bad opcode 0x%02X.", $frame["opcode"]),
			...
			if ($frame["rsv1"] || $frame["rsv2"] || $frame["rsv3"]) {
				return array("success"   => false,
				             "error"     => self::WSTranslate("Invalid frame detected.  One or more reserved extension bits are set."),
			...
			if ($frame["opcode"] >= 0x08) {
				...
				if (!$frame["fin"]) {
					return array("success"   => false,
					             "error"     => self::WSTranslate("Invalid frame detected.  Fragmented control frame was received."),
				...
				if ($frame["opcode"] === self::FRAMETYPE_CONNECTION_CLOSE) {
					if ($this->state === self::STATE_CLOSE) {
						...
						return array("success"   => false,
						             "error"     => self::WSTranslate("Connection closed by peer."),
				...
				if ($lastcompleted) {
					...
					if ($frame["opcode"] === self::FRAMETYPE_CONTINUATION) {
						return array("success"   => false,
						             "error"     => self::WSTranslate("Invalid frame detected.  Fragment continuation frame was received at the start of a fragment."),
					...
					if ($frame["opcode"] !== self::FRAMETYPE_CONTINUATION) {
						return array("success"   => false,
						             "error"     => self::WSTranslate("Invalid frame detected.  Fragment continuation frame was not received for a fragment."),
				...
				if ($this->maxreadmessagesize !== false && strlen($this->readmessages[count($this->readmessages) - 1]["payload"]) > $this->maxreadmessagesize) {
					return array("success"   => false,
					             "error"     => self::WSTranslate("Peer sent a single message exceeding %s bytes of data.", $this->maxreadmessagesize),
