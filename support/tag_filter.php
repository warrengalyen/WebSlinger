...
class TagFilterStream {
	...
	public function Init($options = array()) {
		...
		if (!isset($options["tag_name_map"])) {
			$options["tag_name_map"] = array();
		}
		if (!isset($options["untouched_tag_attr_keys"])) {
			$options["untouched_tag_attr_keys"] = array();
		}
		if (!isset($options["void_tags"])) {
			$options["void_tags"] = array();
		}
		if (!isset($options["alt_tag_content_rules"])) {
			$options["alt_tag_content_rules"] = array();
		}
		if (!isset($options["pre_close_tags"])) {
			$options["pre_close_tags"] = array();
		}
			...
			if ($tag) {
				...
				for ($x = $startpos; $x < $cy; $x ++) {
					...
					if (($val >= $a && $val <= $z) || ($val >= $a2 && $val <= $z2)) {
						if ($x > $cx + 1) {
							$prefix = ltrim(substr($content, $cx + 1, $x - $cx - 1));
						}
						$startpos = $x;
						...
						if (substr($prefix, 0, 3) !== "!--") {
							...
							if ($this->options["keep_comments"]) {
								...
								if (isset($this->options["content_callback"]) && is_callable($this->options["content_callback"])) {
									call_user_func_array($this->options["content_callback"], array(
										$this->stack,
										$result,
										&$content2,
										$this->options
				...
				for (; $cx < $cy; $cx ++) {
					...
					if ($val > 127) {
						$parse = true;
					} else if (!(($val >= $a && $val <= $z) || ($val >= $a2 && $val <= $z2) || ($cx > $startpos && (($val >= $zero && $val <= $nine) || $val == $hyphen || $val == $underscore || $val == $period)) || ($this->options["allow_namespaces"] && $val == $colon))) {
						break;
					}
				}
					...
					do {
						...
						if ($found) {
							do {
								// Let a callback handle any necessary changes.
								$attrs = array();
								if (isset($this->options["tag_callback"]) && is_callable($this->options["tag_callback"])) {
									$funcresult = call_user_func_array($this->options["tag_callback"], array(
										$this->stack,
										&$result,
										false,
								...
								if ($info["close_tag"] && $funcresult["keep_tag"]) {
									$result .= "</" . $info["out_tag_name"] . ">" . $info["post_tag"];
								}
...
//echo "Content:\n" . $content . "\n";
							...
							if ($content{$x} === ">" || $content{$x} === "<") {
								...
								if ($pos === false) {
									...
									if (preg_match('/<\s*\/\s*' . $tagname . '(\s*|\s+.+?)>/is', strtolower($keyname)) || (count($this->stack) && preg_match('/<\s*\/\s*' . $this->stack[0]["tag_name"] . '(\s*|\s+.+?)>/is', strtolower($keyname)))) {
										...
										if ($this->options["allow_namespaces"]) {
								...
								if (($val >= $a && $val <= $z) || ($val >= $a2 && $val <= $z2)) {
									...
									if (!isset($this->options["untouched_tag_attr_keys"][$tagname]) && $this->options["lowercase_attrs"]) {
										$keyname = strtolower($keyname);
									}
						...
						if ($state === "name") {
							...
							if (isset($this->options["process_attrs"][$keyname])) {
								...
								if ($type === "classes") {
									...
									$pos   = strpos($value, ":");
									if ($pos !== false) {
										$value = preg_replace('/[^a-z]/', "", strtolower(substr($value, 0, $pos))) . substr($value, $pos);
									}
								}
							}
				...
				if (isset($this->options["tag_name_map"][$prefix . $tagname])) {
					$outtagname = $tagname = $this->options["tag_name_map"][$prefix . $tagname];
				}

				if ($tagname != "") {
					if ($open) {
						if ($voidtag && isset($this->options["void_tags"][$tagname])) {
							$voidtag = false;
						}
						...
						if (!$open) {
							...
							if ($found) {
								do {
									...
									if (!isset($funcresult["keep_tag"]) || ($info["close_tag"] && $info["open_tag"] == "")) {
										$funcresult["keep_tag"] = true;
									}
...
//exit();
				...
				if (count($this->stack) && isset($this->options["alt_tag_content_rules"][$this->stack[0]["tag_name"]]) && is_callable($this->options["alt_tag_content_rules"][$this->stack[0]["tag_name"]])) {
					...
					if (call_user_func_array($this->options["alt_tag_content_rules"][$this->stack[0]["tag_name"]], array(
						$this->stack,
						$this->final,
						&$tag,
						&$content,
...
class TagFilterNode {
	...
	public function __set($key, $val) {
		if (isset($this->tfn->nodes[$this->id]) && isset($this->tfn->nodes[$this->id]["attrs"])) {
			...
			} else if (is_array($this->tfn->nodes[$this->id]["attrs"][$key])) {
	...
	public function AddClass($name, $attr = "class") {
		if (isset($this->tfn->nodes[$this->id]) && isset($this->tfn->nodes[$this->id]["attrs"])) {
			if (!isset($this->tfn->nodes[$this->id]["attrs"][$attr]) || !is_array($this->tfn->nodes[$this->id]["attrs"][$attr])) {
				$this->tfn->nodes[$this->id]["attrs"][$attr] = array();
			}
...
class TagFilterNodes {
	...
	public static function MakeValidSelector($query) {
		if (!is_array($query)) {
			...
			$result = array(
				"success" => true,
				"tokens"  => TagFilter::ReorderSelectorTokens(array_reverse($query), true)
			);
		}
		...
		foreach ($result["tokens"] as $num => $rules) {
			foreach ($rules as $num2 => $rule) {
				if ($rule["type"] === "pseudo-class") {
					if ($rule["pseudo"] === "link") {
						$result["tokens"][$num][$num2] = array(
							"not"       => false,
							"type"      => "element",
							"namespace" => false,
							"tag"       => "a"
						);
					} else if ($rule["pseudo"] === "disabled") {
						$result["tokens"][$num][$num2] = array(
							"not"       => false,
							"type"      => "attr",
							"namespace" => false,
		...
		$tokens = TagFilter::ReorderSelectorTokens(array_reverse($result["tokens"]), true, array(
			"element"        => array(),
			"id"             => array(),
			"class"          => array(),
			"attr"           => array(),
			"pseudo-class"   => array(),
			"pseudo-element" => array()
		), false);
			...
			for ($x = 0; $x < $y; $x ++) {
				...
				switch ($rules[$x]["type"]) {
					case "id":
						...
						break;
					case "element":
						$str   .= ($rules[$x]["namespace"] !== false ? $rules[$x]["namespace"] . "|" : "") . strtolower($rules[$x]["tag"]);
						$valid = true;
						break;
					case "class":
						$str   .= "." . $rules[$x]["class"];
						...
						break;
					case "attr":
						$str   .= "[" . ($rules[$x]["namespace"] !== false ? $rules[$x]["namespace"] . "|" : "") . strtolower($rules[$x]["attr"]) . ($rules[$x]["cmp"] !== false ? $rules[$x]["cmp"] . "\"" . str_replace("\"", "\\\"", $rules[$x]["val"]) . "\"" : "") . "]";
						$valid = true;
						break;
					case "pseudo-class":
					{
						...
						if ($valid && substr($rules[$x]["pseudo"], 0, 4) === "nth-" && (!isset($rules[$x]["a"]) || !isset($rules[$x]["b"]))) {
							$valid = false;
						}
	...
	public function Find($query, $id = 0, $cachequery = true, $firstmatch = false) {
		...
		if (!isset($this->nodes[$id])) {
			return array("success" => false, "error" => "Invalid initial ID.", "errorcode" => "invalid_init_id");
		}
		...
		do {
			if (!$pos && $this->nodes[$id]["type"] === "element") {
				...
				for ($x = 0; $x < $numrules; $x ++) {
					...
					$y   = count($rules[$x]);
					for ($x2 = 0; $x2 < $y; $x2 ++) {
						if (isset($rules[$x][$x2]["namespace"]) && $rules[$x][$x2]["namespace"] !== false && $rules[$x][$x2]["namespace"] !== "*" && (($rules[$x][$x2]["namespace"] === "" && strpos($this->nodes[$id2]["tag"], ":") !== false) || ($rules[$x][$x2]["namespace"] !== "" && strcasecmp(substr($this->nodes[$id2]["tag"], 0, strlen($rules[$x][$x2]["namespace"]) + 1), $rules[$x][$x2]["namespace"] . ":") !== 0))) {
							$backtrack = true;
						} else {
							switch ($rules[$x][$x2]["type"]) {
								case "id":
									$backtrack = (!isset($this->nodes[$id2]["attrs"]["id"]) || $this->nodes[$id2]["attrs"]["id"] !== $rules[$x][$x2]["id"]);
									break;
								case "element":
									$backtrack = (strcasecmp($this->nodes[$id2]["tag"], (isset($rules[$x][$x2]["namespace"]) && $rules[$x][$x2]["namespace"] !== false ? $rules[$x][$x2]["namespace"] . ":" : "") . $rules[$x][$x2]["tag"]) !== 0);
									break;
								case "class":
									$backtrack = (!isset($this->nodes[$id2]["attrs"]["class"]) || !isset($this->nodes[$id2]["attrs"]["class"][$rules[$x][$x2]["class"]]));
									break;
								case "attr":
								{
									$attr = strtolower($rules[$x][$x2]["attr"]);
									if (!isset($this->nodes[$id2]["attrs"][$attr])) {
										$backtrack = true;
									} else {
										...
										switch ($rules[$x][$x2]["cmp"]) {
											case "=":
												$backtrack = ($val !== $rules[$x][$x2]["val"]);
												break;
											case "^=":
												$backtrack = ($rules[$x][$x2]["val"] === "" || substr($val, 0, strlen($rules[$x][$x2]["val"])) !== $rules[$x][$x2]["val"]);
												break;
											case "$=":
												$backtrack = ($rules[$x][$x2]["val"] === "" || substr($val, - strlen($rules[$x][$x2]["val"])) !== $rules[$x][$x2]["val"]);
												break;
											case "*=":
												$backtrack = ($rules[$x][$x2]["val"] === "" || strpos($val, $rules[$x][$x2]["val"]) === false);
												break;
											case "~=":
												$backtrack = ($rules[$x][$x2]["val"] === "" || strpos($rules[$x][$x2]["val"], " ") !== false || strpos(" " . $val . " ", " " . $rules[$x][$x2]["val"] . " ") === false);
												break;
											case "|=":
												$backtrack = ($rules[$x][$x2]["val"] === "" || ($val !== $rules[$x][$x2]["val"] && substr($val, 0, strlen($rules[$x][$x2]["val"]) + 1) !== $rules[$x][$x2]["val"] . "-"));
												break;
											default:
												$backtrack = false;
												break;
								...
								case "pseudo-class":
								{
									...
									if ($nth && (!isset($rules[$x][$x2]["a"]) || !isset($rules[$x][$x2]["b"]))) {
										return array(
											"success"   => false,
											"error"     => "Pseudo-class ':" . $rules[$x][$x2]["pseudo"] . "(n)' requires an expression for 'n'.",
											"errorcode" => "missing_pseudo_class_expression"
										);
									}
									...
									switch ($rules[$x][$x2]["pseudo"]) {
										...
										default:
											return array(
												"success"   => false,
												"error"     => "Unknown/Unsupported pseudo-class ':" . $rules[$x][$x2]["pseudo"] . "'.",
												"errorcode" => "unknown_unsupported_pseudo_class"
											);
									}
								...
								case "pseudo-element":
									return array(
										"success"   => false,
										"error"     => "Pseudo-elements are not supported.  Found '::" . $rules[$x][$x2]["pseudo"] . "'.",
										"errorcode" => "unsupported_selector_type"
									);
								case "combine":
								{
									switch ($rules[$x][$x2]["combine"]) {
										...
										default:
											return array(
												"success"   => false,
												"error"     => "Unknown combiner " . $rules[$x][$x2]["pseudo"] . ".",
												"errorcode" => "unknown_combiner"
											);
									}
								...
								default:
									return array(
										"success"   => false,
										"error"     => "Unknown selector type '" . $rules[$x][$x2]["type"] . "'.",
										"errorcode" => "unknown_selector_type"
									);
							}
						}

						if (isset($rules[$x][$x2]["not"]) && $rules[$x][$x2]["not"]) {
							$backtrack = !$backtrack;
						}
	...
	public function Filter($ids, $query, $cachequery = true) {
		...
		if (isset($ids["success"])) {
			...
			if (!isset($ids["ids"])) {
				return array("success" => false, "error" => "Bad filter input.", "invalid_filter_ids");
			}
		...
		do {
			...
			if ($pos >= $maxpos) {
				if ($this->nodes[$id]["type"] === "element" && is_array($this->nodes[$id]["children"])) {
					if (($include || $rootid != $id) && isset($types[$this->nodes[$id]["type"]])) {
						$result .= "</" . $this->nodes[$id]["tag"] . ">";
					}
				}

				if ($this->nodes[$id]["type"] === "element" && isset($options["post_elements"][$this->nodes[$id]["tag"]])) {
					$result .= $options["post_elements"][$this->nodes[$id]["tag"]];
				}
	...
	public function GetChildren($id, $objects = false) {
		if (!isset($this->nodes[$id]) || !isset($this->nodes[$id]["children"]) || !is_array($this->nodes[$id]["children"])) {
			return false;
		}
	...
	public function GetNextSibling($id) {
		if ($id === false || !isset($this->nodes[$id]) || $this->nodes[$id]["parentpos"] >= count($this->nodes[$this->nodes[$id]["parent"]]["children"]) - 1) {
			return false;
		}
	...
	public function Move($src, $newpid, $newpos) {
		...
		if ($src instanceof TagFilterNodes) {
			...
			switch ($src["type"]) {
				case "element":
				{
					if (!isset($src["tag"]) || !isset($src["attrs"]) || !is_array($src["attrs"]) || !isset($src["children"])) {
						return false;
					}
	...
	public function SplitAt($ids, $keepidparents = false) {
		...
		do {
			if (!$pos) {
				...
				if ($id) {
					self::SplitAt_CopyNode($newnodes, $newpid, $this->nodes[$id]);
				}
			}
...
class TagFilter {
	...
	public static function GetHTMLOptions() {
		$result = array(
			...
			"pre_close_tags"          => array(
				...
				"li" => array(
					"_limit" => array("ul" => true, "ol" => true, "menu" => true, "dir" => true),
					"li"     => true
				),
			),
			"process_attrs"           => array(
	...
	public static function ExplodeTagCallback($stack, &$content, $open, $tagname, &$attrs, $options) {
		if ($open) {
			...
			if (!isset($options["void_tags"][$tagname])) {
				array_unshift($options["data"]->stackmap, $options["nodes"]->nextid);
			}
	...
	public static function ExplodeContentCallback($stack, $result, &$content, $options) {
		...
		if ($parentpos && $options["nodes"]->nodes[$options["nodes"]->nodes[$pid]["children"][$parentpos - 1]]["type"] == $type) {
			$options["nodes"]->nodes[$options["nodes"]->nodes[$pid]["children"][$parentpos - 1]]["text"] .= $content;
		} else {
			$options["nodes"]->nodes[$options["nodes"]->nextid] = array(
				"type"      => $type,
				"text"      => $content,
	...
	public static function HTMLPurifyTagCallback($stack, &$content, $open, $tagname, &$attrs, $options) {
		if ($open) {
			...
			if (isset($attrs["src"]) && substr($attrs["src"], 0, 11) === "javascript:") {
				return array("keep_tag" => false, "keep_interior" => false);
			}
			if (isset($attrs["href"]) && substr($attrs["href"], 0, 11) === "javascript:") {
			...
			if (!isset($options["htmlpurify"]["allowed_attrs"][$tagname])) {
				...
				foreach ($attrs as $attr => $val) {
					if (!isset($options["htmlpurify"]["allowed_attrs"][$tagname][$attr])) {
						unset($attrs[$attr]);
					}
				}
			}
			...
			if (isset($attrs["class"])) {
				if (!isset($options["htmlpurify"]["allowed_classes"][$tagname])) {
					unset($attrs["class"]);
				} else {
					foreach ($attrs["class"] as $class) {
						if (!isset($options["htmlpurify"]["allowed_classes"][$tagname][$class])) {
	...
	public static function NormalizeHTMLPurifyOptions($purifyopts) {
		if (!isset($purifyopts["allowed_tags"])) {
			$purifyopts["allowed_tags"] = array();
		}
		if (!isset($purifyopts["allowed_attrs"])) {
			$purifyopts["allowed_attrs"] = array();
		}
		if (!isset($purifyopts["required_attrs"])) {
			$purifyopts["required_attrs"] = array();
		}
		if (!isset($purifyopts["allowed_classes"])) {
			$purifyopts["allowed_classes"] = array();
		}
		if (!isset($purifyopts["remove_empty"])) {
			$purifyopts["remove_empty"] = array();
		}
		...
		foreach ($purifyopts["allowed_attrs"] as $key => $val) {
			$purifyopts["allowed_attrs"][$key] = self::Internal_NormalizeHTMLPurifyOptions($val);
		}
		foreach ($purifyopts["required_attrs"] as $key => $val) {
			$purifyopts["required_attrs"][$key] = self::Internal_NormalizeHTMLPurifyOptions($val);
		}
		foreach ($purifyopts["allowed_classes"] as $key => $val) {
			$purifyopts["allowed_classes"][$key] = self::Internal_NormalizeHTMLPurifyOptions($val);
		}
	...
	public static function ReorderSelectorTokens(
		...
		if (count($tokens) && !isset($tokens[0]["type"]) && isset($tokens[0][0]["type"])) {
			...
			foreach ($tokens as $rules) {
				if (count($tokens2)) {
					$tokens2[] = array("type" => "combine", "combine" => "or");
				}
				$rules = array_reverse($rules);
			...
			if ($token["type"] != "combine") {
				...
				foreach ($selector as $vals) {
					foreach ($vals as $token2) {
						if (($endnots && $token2["not"]) || (!$endnots && !$token2["not"])) {
		...
		if ($splitrules) {
			...
			if (count($result) && $result[0]["type"] == "combine" && $result[0]["combine"] == "or") {
				array_shift($result);
			}
		}
			...
			switch ($state) {
				...
				case "selector_ident_result":
				{
					switch ($token["type"]) {
							...
							if ($state2 == "attr") {
								...
								if ($cx >= $cy || $query{$cx} == "]") {
									...
									if ($query{$cx} == "=") {
										...
										return array(
											"success"    => false,
											"error"      => "Unknown or invalid attribute comparison operator '" . $query{$cx} . "' detected at position " . $cx . ".",
											"errorcode"  => "invalid_attr_compare",
											"selector"   => $query,
											"startpos"   => $currcx,
											"pos"        => $cx,
											"state"      => $currstate,
											"tokens"     => self::ReorderSelectorTokens(array_slice($tokens, 0, $lastor), $splitrules),
						...
						case "pseudo-element":
						{
							...
							if ($ident === "first-line" || $ident === "first-letter" || $ident === "before" || $ident === "after") {
								$token["type"] = "pseudo-element";
							}
							...
							if ($token["type"] == "pseudo-class" && $ident == "not") {
								if ($token["not"]) {
									return array(
										"success"    => false,
										"error"      => "Invalid :not() embedded inside another :not() detected at position " . $cx . ".",
								...
								if ($cx >= $cy || $query{$cx} != "(") {
									return array(
										"success"    => false,
										"error"      => "Missing '(' detected at position " . $cx . ".",
										"errorcode"  => "invalid_not",
										"selector"   => $query,
										"startpos"   => $currcx,
										"pos"        => $cx,
				...
				case "negate_close":
				{
					...
					if ($cx < $cy && $query{$cx} != ")") {
						return array(
							"success"    => false,
							"error"      => "Invalid :not() close character '" . $query{$cx} . "' detected at position " . $cx . ".",
				...
				case "pseudo_expression":
				{
					...
					if ($query{$cx} == ")") {
						if (substr($token["pseudo"], 0, 4) === "nth-") {
							...
							if (substr($exp, 0, 2) == "-n") {
								$exp = "-1n" . substr($exp, 2);
							} else if (substr($exp, 0, 2) == "+n") {
								$exp = "1n" . substr($exp, 2);
							} else if (substr($exp, 0, 1) == "n") {
								$exp = "1n" . substr($exp, 1);
							}
				...
				case "ident_name":
				{
					...
					for (; $cx < $cy; $cx ++) {
						...
						if (($val >= $a && $val <= $z) || ($val >= $a2 && $val <= $z2) || $val == $underscore || $val > 127) {
							...
							if (!$range) {
								return array(
									"success"    => false,
									"error"      => "Invalid identifier character '" . $query{$cx} . "' detected at position " . $cx . ".",
							...
							if ($allownamespace && $cx + 1 < $cy && $query{$cx + 1} == "|") {
								...
								if ($ident != "") {
									return array(
										"success"    => false,
										"error"      => "Invalid identifier wildcard character '*' detected at position " . $cx . ".",
					...
					if ($ident == "") {
						return array(
							"success"    => false,
							"error"      => "Missing or invalid identifier at position " . $cx . ".",
				...
				case "combine":
				{
					...
					if ($cx < $cy) {
						...
						if (!count($tokens) || $tokens[count($tokens) - 1]["type"] == "combine") {
							return array(
								"success"    => false,
								"error"      => "Invalid combiner '" . $token["type"] . "' detected at position " . $cx . ".",
			...
			if ($stack[$x]["tag_name"] === $tagname) {
				...
				foreach ($attrs as $key => $val) {
					if (!isset($stack[$x]["attrs"][$key])) {
						$found = false;
					} else if (is_string($stack[$x]["attrs"][$key]) && is_string($val) && stripos($stack[$x]["attrs"][$key], $val) === false) {
						$found = false;
						...
						foreach ($val as $val2) {
							if ($val2 !== "" && !isset($stack[$x]["attrs"][$key][$val2])) {
								$found = false;
							}
						}
					}
