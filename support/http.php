...
class HTTP {
	...
	public static function GetUserAgent($type) {
		...
		if ($type == "ie6") {
			...
			return "Mozilla/5.0 (Windows NT 6.1; WOW64; rv:38.0) Gecko/20100101 Firefox/38.0";
		} else if ($type == "opera") {
		...
		while (strpos($state["data"], "\n") === false) {
			...
			if ($data2 === false || $data2 === "") {
				if (feof($state["fp"])) {
					...
					return array(
						"success"   => false,
						"error"     => self::HTTPTranslate("Non-blocking read returned no data."),
						"errorcode" => "no_data"
					);
				} else if ($data2 === false) {
					return array(
			...
			if ($pos === false) {
				...
				if (self::StreamTimedOut($state["fp"])) {
					return array(
						"success"   => false,
						"error"     => self::HTTPTranslate("Underlying stream timed out."),
			...
			if (isset($state["options"]["readlinelimit"]) && strlen($state["data"]) + $pos > $state["options"]["readlinelimit"]) {
				return array(
					"success"   => false,
			...
			if (isset($state["options"]["recvlimit"]) && $state["options"]["recvlimit"] < $state["rawsize"]) {
				return array(
					"success"   => false,
					"error"     => self::HTTPTranslate("Received data exceeded limit."),
			...
			if (isset($state["options"]["debug_callback"]) && is_callable($state["options"]["debug_callback"])) {
				call_user_func_array($state["options"]["debug_callback"], array(
					"rawrecv",
					$data2,
	...
	private static function ProcessState__ReadBodyData(&$state) {
		while ($state["sizeleft"] === false || $state["sizeleft"] > 0) {
			...
			if ($state["result"]["response"]["code"] == 100 || !isset($state["options"]["read_body_callback"]) || !is_callable($state["options"]["read_body_callback"])) {
				$state["result"]["body"] .= self::GetDecodedBody($state["autodecode_ds"], $data2);
			} else if (!call_user_func_array($state["options"]["read_body_callback"], array(
			...
			if (isset($state["options"]["recvratelimit"])) {
				...
				if (microtime(true) < $state["waituntil"]) {
					return array(
						"success"   => false,
						"error"     => self::HTTPTranslate("Rate limit for non-blocking connection has not been reached."),
						"errorcode" => "no_data"
					);
				}
			}
	...
	private static function ProcessState__WriteData(&$state, $prefix) {
		if ($state[$prefix . "data"] !== "") {
			...
			if ($result === false || feof($state["fp"])) {
				return array(
					"success"   => false,
					"error"     => self::HTTPTranslate("A fwrite() failure occurred.  Most likely cause:  Connection failure."),
			...
			if ($state["async"] && strlen($state[$prefix . "data"])) {
				return array(
					"success"   => false,
					"error"     => self::HTTPTranslate("Non-blocking write did not send all data."),
					"errorcode" => "no_data"
				);
			}
		}
	...
	public static function ProcessState(&$state) {
		...
		if ($state["type"] === "request") {
				...
				switch ($state["state"]) {
					case "connecting":
					{
						if (function_exists("stream_select") && $state["async"]) {
							...
							if (!count($writefp)) {
								return array(
									"success"   => false,
									"error"     => self::HTTPTranslate("Connection not established yet."),
									"errorcode" => "no_data"
								);
							}
						}
					...
					case "proxy_connect_response":
					{
						...
						if (substr($result["response"]["code"], 0, 1) != "2") {
							return self::CleanupErrorState($state, array(
								"success"   => false,
								"error"     => self::HTTPTranslate("Expected a 200 response from the CONNECT request.  Received:  %s.", $result["response"]["line"]),
								"info"      => $result,
					...
					case "send_data":
					{
						...
						if (isset($state["options"]["write_body_callback"]) && is_callable($state["options"]["write_body_callback"])) {
							if ($state["bodysize"] === false || $state["bodysize"] > 0) {
								...
								$result    = call_user_func_array($state["options"]["write_body_callback"], array(
									&$state["data"],
									&$bodysize2,
									&$state["options"]["write_body_callback_opts"]
								));
								if (!$result || ($state["bodysize"] !== false && strlen($state["data"]) > $state["bodysize"])) {
									return self::CleanupErrorState($state, array(
										"success"   => false,
							...
							if ($state["currentfile"] === false && count($state["options"]["files"])) {
								...
								if (!isset($state["currentfile"]["datafile"])) {
									...
									if ($state["currentfile"]["fp"] === false) {
										return self::CleanupErrorState($state, array(
											"success"   => false,
											"error"     => self::HTTPTranslate("The file '%s' does not exist.", $state["currentfile"]["datafile"]),
							...
							if ($state["currentfile"] !== false && isset($state["currentfile"]["fp"])) {
								...
								if ($state["currentfile"]["filesize"] >= 65536) {
									...
									if ($data2 === false || strlen($data2) !== 65536) {
										return self::CleanupErrorState($state, array(
											"success"   => false,
											"error"     => self::HTTPTranslate("A read error was encountered with the file '%s'.", $state["currentfile"]["datafile"]),
											"errorcode" => "file_read"
										));
									}
					...
					case "receive_switch":
					{
						if (function_exists("stream_select") && $state["async"]) {
							...
							if (!count($readfp)) {
								return array(
									"success"   => false,
									"error"     => self::HTTPTranslate("Connection not fully established yet."),
									"errorcode" => "no_data"
								);
							}
						}
				...
				switch ($state["state"]) {
					...
					case "request_line":
					{
						...
						if ($pos === false) {
							return self::CleanupErrorState($state, array(
								"success"   => false,
								"error"     => self::HTTPTranslate("Unable to retrieve request line."),
					...
					{
						$result = self::ProcessState__ReadLine($state);
						if (!$result["success"] && ($state["state"] === "headers" || ($result["errorcode"] !== "stream_read_error" && $result["errorcode"] !== "peer_disconnected"))) {
							return self::CleanupErrorState($state, $result);
						}
						...
						$state["rawrecvheadersize"] += $pos + 1;
						if ($header != "") {
							if ($state["lastheader"] != "" && (substr($header, 0, 1) == " " || substr($header, 0, 1) == "\t")) {
								$state["result"]["headers"][$state["lastheader"]][count($state["result"]["headers"][$state["lastheader"]]) - 1] .= $header;
							} else {
								$pos = strpos($header, ":");
								if ($pos === false) {
									$pos = strlen($header);
								...
								if (!isset($state["result"]["headers"][$state["lastheader"]])) {
									$state["result"]["headers"][$state["lastheader"]] = array();
								}
								$state["result"]["headers"][$state["lastheader"]][] = ltrim(substr($header, $pos + 1));
							}

							$state["numheaders"] ++;
							if (isset($state["options"]["maxheaders"]) && $state["numheaders"] > $state["options"]["maxheaders"]) {
								return self::CleanupErrorState($state, array(
									"success"   => false,
									"error"     => self::HTTPTranslate("The number of headers exceeded the limit."),
							...
							if ($state["result"]["response"]["code"] != 100 && isset($state["options"]["read_headers_callback"]) && is_callable($state["options"]["read_headers_callback"])) {
								if (!call_user_func_array($state["options"]["read_headers_callback"], array(
									&$state["result"][($state["client"] ? "response" : "request")],
									&$state["result"]["headers"],
									&$state["options"]["read_headers_callback_opts"]
								))) {
									return self::CleanupErrorState($state, array(
										"success"   => false,
							...
							if ($state["state"] === "body_chunked_headers") {
								...
								if ($state["result"]["response"]["code"] == 100 || $state["result"]["response"]["code"] == 101) {
									...
									if ($state["autodecode"] && !isset($state["result"]["headers"]["Content-Encoding"]) || (strtolower($state["result"]["headers"]["Content-Encoding"][0]) != "gzip" && strtolower($state["result"]["headers"]["Content-Encoding"][0]) != "deflate")) {
									...
									if (!$state["client"] && $state["state"] !== "done") {
										return array(
											"success"   => false,
											"error"     => self::HTTPTranslate("Intermission for adjustments to limits."),
											"errorcode" => "no_data"
										);
									}
								}
					...
					{
						$result = self::ProcessState__ReadBodyData($state);
						if (!$result["success"] && (($state["sizeleft"] !== false && $state["sizeleft"] > 0) || ($state["sizeleft"] === false && $result["errorcode"] !== "stream_read_error" && $result["errorcode"] !== "peer_disconnected" && $result["errorcode"] !== "stream_timeout_exceeded"))) {
							return self::CleanupErrorState($state, $result);
						}
			...
			if ($state["close"] || ($state["client"] && isset($state["result"]["headers"]["Connection"]) && strtolower($state["result"]["headers"]["Connection"][0]) === "close")) {
				fclose($state["fp"]);
			} else {
				$state["result"]["fp"] = $state["fp"];
			}
			...
			return self::CleanupErrorState($state, array(
				"success"   => false,
				"error"     => self::HTTPTranslate("Invalid 'type' in state tracker."),
	...
	public static function RetrieveWebpage($url, $options = array()) {
		...
		if ($host == "") {
			return array("success" => false, "error" => self::HTTPTranslate("Invalid URL."));
		}
		...
		if ($useproxy) {
			...
			if ($proxyconnect) {
				...
				if (isset($options["headers"]["User-Agent"])) {
					$proxydata .= "User-Agent: " . $options["headers"]["User-Agent"] . "\r\n";
				}
		...
		if (!isset($options["method"])) {
			if ((isset($options["write_body_callback"]) && is_callable($options["write_body_callback"])) || isset($options["body"])) {
				$options["method"] = "PUT";
		...
		if (isset($options["write_body_callback"]) && is_callable($options["write_body_callback"])) {
			...
			call_user_func_array($options["write_body_callback"], array(
				&$body,
				&$bodysize,
		...
		if (($bodysize === false && strlen($body) > 0) || ($bodysize !== false && $bodysize < strlen($body))) {
			$bodysize = strlen($body);
		}
		...
		} else if ($bodysize > 0 || $body != "" || $options["method"] == "POST") {
