...
class DeflateStream {
	...
	private function ProcessInput($final = false) {
		// Automatically determine the type fof data on the header signature.
		...
		if ($this->options["type"] == "gzip") {
			...
			if (!$this->options["header"]) {
				if (strlen($this->indata) >= 2) {
					...
					// Compression method 'deflate' ($cm = 8), window size - 8 ($cminfo < 8), no preset dictionaries ($flg bit 5), checksum validates.
