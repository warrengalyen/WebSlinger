...
function SetReverseManifestPath($key) {
	...
	$x = 0;
	while (isset($manifestrev[strtolower($path . ($x ? "_" . ($x + 1) : "") . $ops[$key]["ext"])]) || is_dir($path . ($x ? "_" . ($x + 1) : "") . $ops[$key]["ext"])) {
		$x ++;
	}
...
while ($result["success"]) {
	...
	foreach ($result["removed"] as $key => $info) {
		if (!$info["result"]["success"]) {
			$ops[$key]["retries"] --;
			if ($ops[$key]["retries"]) {
				$ops[$key]["web"]->ProcessAsync($helper, $key, null, $key, $info["tempoptions"]);
			}
			...
			while (count($process)) {
				...
				if ($opsdata[$key2]["httpcode"] >= 400) {
					echo "[" . number_format(count($ops), 0) . " ops] Finalizing '" . $key2 . "'.\n";
				} else {
