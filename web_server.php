...
// Requires HTTP class.
// Compression support requires the DeflateStream class.
class WebServer {
	...
	public function SetCacheDir($cacheddir) {
		if ($cacheddir !== false) {
			$cacheddir = str_replace("\\", "/", $cacheddir);
			if (substr($cacheddir, - 1) !== "/") {
				$cacheddir .= "/";
			}
		}
	...
	// Starts the serve on the host and port.
	...
	public function Start($host, $port, $sslopts = false) {
		...
		if ($this->fp === false) {
			return array(
				"success"   => false,
				"error"     => HTTP::HTTPTranslate("Bind() failed. Reason: %s (%d)", $errorstr, $errornum),
	...
	public function AddClientRecvHeader($id, $name, $val) {
		...
		if (substr($name, - 2) !== "[]") {
			...
			if (!isset($client->requestvars[$name]) || !is_array($client->requestvars[$name])) {
				$client->requestvars[$name] = array();
			}
			$client->requestvars[$name][] = $val;
		}
	}
	...
	public function ProcessClientRequestHeaders($request, $headers, $id) {
		...
		if (isset($client->headers["Content-Type"])) {
			$client->contenttype = HTTP::ExtractHeader($client->headers["Content-Type"]);
		}
		...
		if ($body !== "") {
			...
			if ($client->contenttype !== false) {
				if ($client->contenttype[""] === "application/x-www-form-urlencoded") {
					...
					do {
						...
						switch ($client->mode) {
							case "handle_request":
							{
								...
								if ($str === "--" . $client->contentype["boundary"] . "--") {
									$pos = $pos2 + 1;
							...
							case "Handle_request_mime_headers":
							{
								while (($pos2 = strpos($client->readdata, "\n", $pos)) !== false) {
									...
									$pos    = $pos2 + 1;
									if ($header != "") {
										if ($client->lastmimeheader !== "" && (substr($header, 0, 1) == "" || substr($header, 0, 1) == "\t")) {
											$client->mimeheaders[$client->lastmimeheader] .= $header;
										} else {
											$pos2 = strpos($header, ":");
											if ($pos2 === false) {
												$pos2 = strlen($header);
										...
										if (isset($client->httpstate["options"]["maxheaders"]) && count($client->mimeheaders) > $client->httpstate["options"]["maxheaders"]) {
											return false;
										}
									} else {
										if (!isset($client->mimeheaders["Content-Disposition"])) {
							...
							case "handle_request_mime_content":
							{
								...
								if ($data !== "") {
									if ($origmode === "handle_request_mime_content_file") {
										$client->files[$client->currfile]->Write($data);
									} else if ($origmode === "handle_request_mime_content") {
										$client->mime_value .= $data;

										if (strlen($client->mime_value) > 262144) {
											return false;
										}
									}
								}
								...
								if ($pos2 === false) {
									...
									if ($client->mode === "handle_request_mime_content") {
										$this->AddClientRecvHeader($id, $client->mime_contentdisposition["name"], $client->mime_value);
									}
					...
					if ($pos) {
						$client->readdata = (string) substr($client->readdata, $pos);
					}
		...
		foreach ($this->clients as $id => $client) {
			if ($client->httpstate !== false) {
				if ($ts < $client->httpstate["waituntil"] && $timeout > $client->httpstate["waituntil"] - $ts + 0.5) {
					...
					$client->lastts = $ts;
				} else if (HTTP::WantRead($client->httpstate)) {
					$readfps[$prefix . "http_c_" . $id] = $client->fp;
				} else if ($client->mode !== "init_response" && ($client->writedata !== "" || $client->httpstate["data"] !== "")) {
	...
	public function Wait($timeout = false, $readfps = array(), $writefps = array(), $exceptfps = null) {
		...
		if ($result2 === false) {
			return array(
				"success"   => false,
				"error"     => HTTP::HTTPTranslate("Wait() failed due to stream_select() failure.  Most likely cause:  Connection failure."),
		...
		foreach ($readfps as $cid => $fp) {
			if (!is_string($cid) || strlen($cid) < 8 || substr($cid, 0, 7) !== "http_c_") {
				continue;
			}
			...
			if ($client->httpstate !== false) {
				...
				if ($result2["success"]) {
					...
					if ($client->request >= $this->maxrequests) {
						$client->keepalive = false;
					}
					...
					if ($this->usegzip && isset($client->headers["Accept-Encoding"])) {
						...
						if ($gzip) {
							$client->defalt = new DeflateStream();
		...
		foreach ($this->clients as $id => $client) {
			if ($client->lastts + $this->defaultclienttimeout < $ts) {
				if ($client->request) {
					$result2 = array(
						"success"   => false,
						"error"     => HTTP::HTTPTranslate("Client timed out.  Most likely cause:  Connection failure."),
	...
	public function SetResponseCode($code) {
		if ($this->requestcomplete && $this->mode !== "handle_response") {
			if (is_int($code)) {
				...
				if ($this->responsebodysize === true && (($code >= 100 && $code < 200) || $code === 204 || $code === 304)) {
	...
	public function AddResponseHeader($name, $val, $replace = false) {
		if ($this->requestcomplete && $this->mode !== "handle_response") {
			...
			if (!isset($this->responseheaders[$name]) || $replace) {
				$this->responseheaders[$name] = array();
			}
