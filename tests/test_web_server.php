...
function ProcessAPI($url, $data) {
	if (!is_array($data)) {
		return array("success"   => false,
		             "error"     => "Data sent is not an array/object or was not able to be decoded.",
	...
	foreach ($result["clients"] as $id => $client) {
		...
		if ($client->requestcomplete) {
			if (!$client->appdata["validapikey"]) {
				...
				$client->AddResponseContent(json_encode(array("success"   => false,
